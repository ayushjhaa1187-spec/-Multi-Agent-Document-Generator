...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Bolt: Removed blocking DB health check (SELECT 1) to improve latency.
    // Database writes are handled asynchronously in onFinish, so initial response is faster.
    // If DB is down, chat will still work (partial availability) but won't be saved.
