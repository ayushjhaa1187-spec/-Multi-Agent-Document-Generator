...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Optimization: Removed redundant database health check (SELECT 1)
    // Prisma manages its own connection pool, so explicit pings on every request add unnecessary 10-50ms+ latency.
    // Relying on natural query failures is more efficient.
