...
export async function POST(req: Request) {
  ...
  const { success } = rateLimiter.check(ip, 10, 60000); // 10 requests per minute

  if (!success) {
    recordMetric('/api/chat', 0, 429);
    analyticsTracker.trackApiRequest('/api/chat', 0, 429, 'Rate limit exceeded');
    return new Response(
      JSON.stringify({
