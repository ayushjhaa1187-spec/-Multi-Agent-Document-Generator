...
export async function POST(req: Request) {
  ...
  try {
    ...
    // to reduce unnecessary network latency (saves ~10-50ms per request).
    // Prisma automatically manages its connection pool, so we rely on natural query failures instead.
