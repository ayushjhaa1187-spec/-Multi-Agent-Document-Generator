...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Prisma manages its own connection pool. Explicit pings on every request add
    // unnecessary network latency (10-50ms+ per request). Relying on natural query
    // failures instead to improve response times.
