 ...
 * Why: During Vercel AI SDK text streaming, the parent `BRDGenerator` component re-renders rapidly with each new token. Without memoization, every existing message in the list re-renders on every chunk, causing an O(N²) rendering bottleneck.
 * Impact: Reduces re-renders of the message list from O(N²) to O(1) for existing messages during active text streaming.
 */
