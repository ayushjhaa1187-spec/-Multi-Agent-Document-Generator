 ...
 * Why: When using Vercel AI SDK text streaming, inline components mapped over the `messages`
 * array trigger O(N²) re-renders on every chunk because React recreates component instances.
 * Impact: Extracting this into a standalone component wrapped in `React.memo()` and defined
 * outside the parent reduces O(N²) operations to O(1) per chunk. Only the actively streaming
 * message will re-render, drastically improving responsiveness during generation.
 */
const MessageItem = memo(({ message, stage }: { message: Message; stage: string }) => (
  <div
  ...
  return prevProps.message.content === nextProps.message.content && prevProps.stage === nextProps.stage;
});
