...
/**
 * ⚡ Bolt Performance Optimization: MessageItem
 *
 * Why: During AI text streaming, the `useChat` messages array updates on every token.
 * Without memoization, every message in the list re-renders for every new token received,
 * causing an O(N^2) rendering performance issue where N is the number of tokens.
 *
 * Impact: Wrapping the message item in React.memo() reduces rendering overhead to O(1) per token,
 * as only the currently streaming message needs to re-render, keeping the UI highly responsive.
 */
const MessageItem = memo(({ message, stage }: { message: Message; stage: 'clarify' | 'generate' }) => (
  <div
