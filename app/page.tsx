 ...
 * Extracted the individual message renderer from the map loop and wrapped it in React.memo().
 * Why: Vercel AI SDK's useChat streams the latest message rapidly. Rendering the entire
 * message list (O(N) elements) on every incoming token update blocks the main thread.
 * Impact: React.memo() prevents older messages from re-rendering, reducing work to O(1)
 * instead of O(N^2) during the total stream period.
 */
