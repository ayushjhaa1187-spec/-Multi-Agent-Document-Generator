...
// ⚡ Bolt Optimization: Memoized chat message component
// By wrapping the individual message in React.memo, we prevent the entire list of
// previous messages from re-rendering every time a new message streams in.
// Expected Impact: Reduces unnecessary component re-renders by ~O(N) for each new chunk,
// significantly improving UI responsiveness during long conversations.
const MemoizedMessage = memo(function MemoizedMessage({ message, stage }: { message: Message, stage: 'clarify' | 'generate' }) {
  return (
    <div
            ...
            <div className="chat-container glass-card p-6 sm:p-8 mb-6 shadow-2xl min-h-[400px] sm:min-h-[500px] max-h-[600px] overflow-y-auto">
              ...
              <div className="space-y-4">
                {messages.map((message) => (
                  <MemoizedMessage key={message.id} message={message} stage={stage} />
                ))}
