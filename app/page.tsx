...
export default function BRDGenerator() {
  ...
  useEffect(() => {
    if (textareaRef.current) {
      ...
      // Add 2px for the top and bottom borders to prevent scrollbar flickering
            ...
            <form onSubmit={handleSubmit} className="glass-card p-6 sm:p-8 shadow-2xl">
              <div className="flex gap-2 sm:gap-4 mb-4 items-end">
                <textarea
                  ...
                  className="flex-1 px-4 sm:px-6 py-3 sm:py-4 bg-white/5 border border-white/20 rounded-xl text-white placeholder-gray-400 focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition-all text-sm sm:text-base overflow-y-auto resize-none"
                  disabled={isLoading}
                  rows={1}
