 ...
 * Why: During Vercel AI SDK text streaming, mapping inline components over the `messages` array triggers O(N²) re-renders on every streamed chunk.
 * Impact: Wrapping the extracted component in `memo()` reduces operations to O(1) by ensuring only the actively streaming message re-renders.
 */
const ChatMessage = memo(({ message, stage }: { message: { id: string, role: string, content: string }, stage: 'clarify' | 'generate' }) => (
  <div
