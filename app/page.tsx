 ...
 * Why: Vercel AI SDK text streaming updates the `messages` state on every chunk. When messages are rendered inline, this causes all historical messages to re-render repeatedly, causing O(N²) operations and significant UI jank during long conversations.
 * Impact: Reduces React render cycles for historical messages to 0 during streaming. Only the actively streaming message re-renders, changing the rendering performance from O(N²) to O(1) per chunk.
 */
const ChatMessage = React.memo(
  ({ message, stage }: { message: Message; stage: string }) => (
    <div
