...
// Extracted inline message rendering into a React.memo() component.
// Why: Vercel AI SDK streams text in chunks, causing the entire messages array to re-render on every chunk (O(N²)).
// Impact: Reduces React render cycle overhead significantly during streaming by only re-rendering the actively streaming message.
const MessageItem = React.memo(({ message, stage }: { message: { id: string, role: string, content: string }, stage: string }) => (
  <div
