...
export default function BRDGenerator() {
  ...
  useEffect(() => {
    if (textareaRef.current) {
      // Store current scroll positions to prevent jumping
            ...
            <form onSubmit={handleSubmit} className="glass-card p-6 sm:p-8 shadow-2xl">
              <div className="flex gap-2 sm:gap-4 mb-4 items-end">
                ...
                <textarea
                  ...
                  className="flex-1 min-w-0 resize-none overflow-y-auto min-h-[52px] max-h-[200px] px-4 sm:px-6 py-3 sm:py-4 bg-white/5 border border-white/20 rounded-xl text-white placeholder-gray-400 focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition-all text-sm sm:text-base"
                  disabled={isLoading}
                  rows={1}
                />
                <button
